...
groups.special <- groups.special[-23,]
groups.special <- groups.special[-5,]
groups.special <- groups.special[-4,]
groups.special <- groups.special[-19,]
groups.special$X
...
constructLogRegModelsFromClusters <- function(data, models_per_k=10,k_range=:5,
      ...
      if(doLogReg == T){
        ...
        for(i in 1:length(all_models)){
          ...
          for(j in 1:length(curr_kmean$size)){
            ...
            # Create a logistic regression model that discriminates in cluster
            # and out of cluster using cross-validation
            # glmnet. famous design. stanford. try different lmda 
...
model_list <- constructLogRegModelsFromClusters(df,saveModels = T,models_per_k = 10,cluster_min=1,
                                                working_directory="~/r_files/6-12-18/21-2clus",
                                                k_range=2, doLogReg=T,special=F)

# Tweak the variables in this function to find the results you would like in cluster analysis.
model_list.special <- constructLogRegModelsFromClusters(df.special,saveModels = T,models_per_k = 10,cluster_min=1,
                                                working_directory="~/r_files/6-12-18/14-holdoutWithLog",
